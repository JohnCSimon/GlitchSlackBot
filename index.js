    ...
    .then(() => {
      ...
      .then((value) => {
        ...
        function modifySteps(user, type, item, counter, message) {
          counter++;
          const step = message.attachments[counter];
          ...
          if (counter == message.attachments.length) { // got all values back, can move on
            ...
            slack.send(message).then((res) => { // on success
              ...
              users[user] = Object.assign({}, message, { ts, channel });
            }, (reason) => { // on failure
              console.log(`An error occurred when responding to event: ${  reason}`);
            });
          } else if (counter < message.attachments.length) {
            let storedStep;
            const valueRef = user + step.event; // sets a user-specific reference used to refer to data in the dynamodb
            ...
            .then((value) => {
              ...
              if (storedStep) {
                ...
                step.color = '#2ab27b';
                step.completed = true;
                datastore.set(valueRef, true).then(() => { // store that the step has been completed in the dynamodb
        ...
        slack.on('/onboarding', (payload) => {
          ...
          const message = getStatusMessage(user_id);
        ...
        slack.on('star_added', 'pin_added', 'reaction_added', (payload) => {
          ...
          const message = getStatusMessage(user);
